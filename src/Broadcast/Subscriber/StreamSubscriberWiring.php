...
/**
 * Wires the broadcast subscriber into the native stream_select() driver.
 *
 * Unix: registers the Unix domain socket as a watched stream; accumulates
 *       fread() chunks until EOF before dispatching (fixes the race condition
 *       where a non-blocking read may return empty before the sender flushes).
 *
 ...
 *        child death.
 */
final class StreamSubscriberWiring
{
    public function __construct(
        private readonly StreamSocketDriver $driver,
    ...
    private function wireUnix(BroadcastBridge $bridge): void
    {
        ...
        $this->driver->addStream($unixServer, function ($unixServer) use ($bridge): void {
            ...
            $buffer = '';
            $this->driver->addStream($conn, function ($conn) use (&$buffer, $bridge): void {
    ...
    private function wireRedis(BroadcastBridge $bridge): void
    {
        ...
        $spawnChild = function () use (&$spawnChild, $bridge): void {
            ...
            $this->driver->addStream($parentSocket, function ($parentSocket) use ($bridge, &$spawnChild): void {
